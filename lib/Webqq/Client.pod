...
=item 发送消息结构
    ...
    $msg = {
        ...
        allow_plugin   => #是否运行插件处理该消息标记，插件之间互相配合需要用到
    }
...
=item on_send_message() :lvalue 
    ...
    因此你不需要再根据$is_success为假时自己在进行重试
...
=item plugin($module)
    ...
    当你需要获取插件的返回值，则需要通过$client->plugin()获取到插件函数引用，
    然后自己执行
...
=item Webqq::Client::Plugin
    ...
    一个简单的客户端插件管理模块，被Webqq::Client继承
    包含几个方法：
    ...
    $plugin_code_ref是Webqq::Client::Plugin::Tes::call()的引用
