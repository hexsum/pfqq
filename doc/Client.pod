...
=item 发送消息结构
    ...
    $msg = {
        ...
        ttl            => #发送消息带有一个ttl值，默认是5，当ttl减为1时会被发送消息队列丢弃
    }
...
=item on_send_message() :lvalue 
    ...
    或者用于发送消息失败后进行重发（腾讯的api接口白天总是不太稳定经常会返回5xx）
    ...
    $client->on_send_message() = sub{
        ...
        if($is_success){
            ...
            #发送失败，把消息重新放回消息队列里，$msg->ttl值默认是5,每次经过队列就会递减
            $client->{send_message_queue}->put($msg);

            #上述的方式用到了$client->{send_message_queue} 这个文档中并没有过多介绍
            #有兴趣可以参见Webqq::Message::Queue;
            #或者采用更规范的方式:
            $client->send_message($msg) if $msg->{type} eq 'message';
            $client->send_group_message($msg) if $msg->{type} eq 'group_message';
            $client->send_sess_message($msg) if $msg->{type} eq 'sess_message';
            #这种方式不好的地方是你需要自己判断消息类型再调用相应的发送方法
        }
